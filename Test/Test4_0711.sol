...
contract TEST4 {
    ...
    */    
    ...
    // * 로그인 기능 - ID, PW를 넣으면 로그인 여부를 알려주는 기능
    // * 비밀번호 5회 이상 오류시 경고 메세지 기능 - 비밀번호 시도 회수가 5회되면 경고 메세지 반환
    function signIn(string memory _id, bytes32 _hash) public returns(string memory){
        if(pw[_id] == _hash) {
    ...
    // * 회원탈퇴 기능 - 회원이 자신의 ID와 PW를 넣고 회원탈퇴 기능을 실행하면 관련 정보 삭제
    function deleteAccount(string memory _id, bytes32 _hash) public {
        require(pw[_id] == _hash, "incorrect");
